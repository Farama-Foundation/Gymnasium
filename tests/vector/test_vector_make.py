...
@pytest.mark.parametrize("num_envs", [1, 3, 10])
def test_vector_make_num_envs(num_envs):
env = gym.make("CartPole-v1", num_envs=num_envs)
assert env.num_envs == num_envs
env.close()

def test_vector_make_asynchronous():
env = gym.make("CartPole-v1", asynchronous=True)
assert isinstance(env, AsyncVectorEnv)
env.close()

python
Copy code
env = gym.make("CartPole-v1", asynchronous=False)
assert isinstance(env, SyncVectorEnv)
env.close()
def test_vector_make_wrappers():
env = gym.make("CartPole-v1", num_envs=2, asynchronous=False)
assert isinstance(env, SyncVectorEnv)
assert len(env.envs) == 2
...
assert all(has_wrapper(sub_env, TransformObservation) is False for sub_env in env.envs)
env.close()

env = gym.make("CartPole-v1", num_envs=2, asynchronous=False, wrappers=lambda _env: TransformObservation(_env, lambda obs: obs * 2))
...
env.close()
def test_vector_make_disable_env_checker():
env = gym.make("CartPole-v1", num_envs=1, asynchronous=False)
assert isinstance(env, SyncVectorEnv)
assert has_wrapper(env.envs[0], PassiveEnvChecker)
env.close()
...
python
Copy code
env = gym.make("CartPole-v1", num_envs=5, asynchronous=False)
assert isinstance(env, SyncVectorEnv)
assert has_wrapper(env.envs[0], PassiveEnvChecker)
assert all(has_wrapper(env.envs[i], PassiveEnvChecker) is False for i in [1, 2, 3, 4])
env.close()

env = gym.make("CartPole-v1", num_envs=3, asynchronous=False, disable_env_checker=True)
assert isinstance(env, SyncVectorEnv)
