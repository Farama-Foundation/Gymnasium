...
@pytest.mark.parametrize(
    ...
    [
        (
            GenericTestEnv(action_space=NESTED_DICT_SPACE, step_fn=generic_step_fn),
            {
        ...
        (
            GenericTestEnv(
                action_space=DOUBLY_NESTED_DICT_SPACE, step_fn=generic_step_fn
            ),
            {
                "box": (NEW_BOX_LOW, NEW_BOX_HIGH),
...
def test_scale_actions_v0_tuple():
    ...
    env = GenericTestEnv(action_space=TUPLE_SPACE, step_fn=generic_step_fn)
...
@pytest.mark.parametrize(
    ...
    [
        (
            GenericTestEnv(action_space=NESTED_TUPLE_SPACE, step_fn=generic_step_fn),
            [None, [None, (NEW_BOX_LOW, NEW_BOX_HIGH)]],
            [BOX_HIGH, [DISCRETE_ACTION, NEW_BOX_HIGH]],
        ),
        (
            GenericTestEnv(
                action_space=DOUBLY_NESTED_TUPLE_SPACE, step_fn=generic_step_fn
            ),
