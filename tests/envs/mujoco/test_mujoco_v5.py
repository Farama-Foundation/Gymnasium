...
# Note: "Humanoid-v4/3" & "Ant-v4/3" fail this test
...
def test_verify_info_xy_velocity_xpos(env_id: str):
    """Asserts that the environment `info['x/y_velocity']` is properly assigned, for the ant environment which uses kinematics for the velocity."""
    env = gym.make(env_id).unwrapped
    assert isinstance(env, MujocoEnv)
    env.reset()
...
def test_verify_info_xy_velocity_com(env_id: str):
    """Asserts that the environment `info['x/y_velocity']` is properly assigned, for the humanoid environment which uses kinematics of Center Of Mass for the velocity."""
...
# Note: all `v4/v3/v2` environments with a healthy reward are fail this test
@pytest.mark.parametrize(
    "env_name",
    [
        "Ant",
...
# Dev Note: This can not be version env parametrized because each env has its own reward function
@pytest.mark.parametrize("version", ["v5"])
...
def test_ant_com(version: str):
    """Verify the kinematic behaviour of the ant."""
    # `env` contains `data : MjData` and `model : MjModel`
    env = gym.make(f"Ant-{version}").unwrapped
    assert isinstance(env, MujocoEnv)
    env.reset()  # randomly initializes the `data.qpos` and `data.qvel`, calls mujoco.mj_forward(env.model, env.data)
...
def test_set_state(version: str):
    ...
    assert np.all(env.data.qpos == new_qpos)
    assert np.all(env.data.qvel == new_qvel)
