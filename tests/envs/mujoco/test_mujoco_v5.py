...
ALL_MUJOCO_ENVS = ['Ant', 'HalfCheetah', 'Hopper', 'Humanoid', 'HumanoidStandup', 'InvertedDoublePendulum', 'InvertedPendulum', "Pusher", "Reacher", "Swimmer", 'Walker2d']


# Note: "HumnanoidStandup-v4" does not have `info`
# Note: "Humnanoid-v4/3" & "Ant-v4/3" fail this test
@pytest.mark.parametrize("env_id", ['Ant-v5', 'HalfCheetah-v5', 'HalfCheetah-v4', 'HalfCheetah-v3', 'Hopper-v5', 'Hopper-v4', 'Hopper-v3', 'Humanoid-v5', 'HumanoidStandup-v5', 'Swimmer-v5', 'Swimmer-v4', 'Swimmer-v3', 'Walker2d-v5', 'Walker2d-v4', 'Walker2d-v3'])
...
# Note: "HumnanoidStandup-v4" does not have `info`
# Note: "Humnanoid-v4/3" & "Ant-v4/3" fail this test
@pytest.mark.parametrize("env_id", ['Ant-v5', 'Humanoid-v5', 'HumanoidStandup-v5', 'Swimmer-v5'])
...
def test_reward_sum(version):
    ...
    assert reward - info["reward_forward"] - info["reward_ctrl"] - info["reward_contact"] - info["reward_survive"] < 1e-14
    ...
    env.reset()
    _, reward, _, _, info = env.step(env.action_space.sample())
    assert reward - info["reward_forward"] - info["reward_ctrl"] - info["reward_survive"] < 1e-14
    ...
    env.reset()
    _, reward, _, _, info = env.step(env.action_space.sample())
    assert reward - info["reward_linup"] - info["reward_quadctrl"] - info["reward_impact"] - 1 < 1e-14

    env = gym.make(f'InvertedDoublePendulum-{version}')
    env.reset()
    _, reward, _, _, info = env.step(env.action_space.sample())
    assert reward - info["reward_survive"] - info["distance_penalty"] - info["velocity_penalty"] < 1e-14
    ...
    env.reset()
    _, reward, _, _, info = env.step(env.action_space.sample())
    assert reward - info["reward_dist"] - info["reward_ctrl"] - info["reward_near"] < 1e-14
...
@pytest.mark.parametrize("env", ['HalfCheetah', 'HumanoidStandup', 'Pusher', 'Reacher', 'Swimmer'])
...
def test_ant_com(version):
    ...
    env = gym.make(f'Ant-{version}')  # `env` contains `data : MjData` and `model : MjModel`
...
def test_set_state(version):
    ...
    env = gym.make(f'Hopper-{version}')
    env.reset()
    new_qpos = np.array([0.00136962, 1.24769787, -0.00459026, -0.00483472, 0.0031327 , 0.00412756])
    new_qvel = np.array([0.00106636, 0.00229497, 0.00043625, 0.00435072, 0.00315854, -0.00497261])
...
@pytest.mark.parametrize("env_id", ['Ant-v5', 'Humanoid-v5', 'Swimmer-v5', 'Swimmer-v4', 'Swimmer-v3'])
def test_distance_from_origin_info(env_id):
    """Verify that `info"distance_from_origin"` is correct"""
    env = gym.make(env_id)
    env.reset()
    _, _, _, _, info = env.step(env.action_space.sample())
    assert info["distance_from_origin"] == np.linalg.norm(env.unwrapped.data.qpos[0:2] - env.init_qpos[0:2])
...
def test_z_distance_from_origin_info(env_id):
    ...
    assert info["z_distance_from_origin"] == env.unwrapped.data.qpos[z_index] - env.init_qpos[z_index]


@pytest.mark.parametrize("env_id", ['Ant-v5', 'HalfCheetah-v5', 'Hopper-v5', 'Humanoid-v5', 'HumanoidStandup-v5', 'InvertedPendulum-v5', 'Swimmer-v5', 'Walker2d-v5'])
def test_observation_structure(env_id):
    ...
    assert env.unwrapped.model.nq == obs_struct.get("skipped_qpos", 0) + obs_struct["qpos"]
...
@pytest.mark.parametrize("env_id", ['Ant-v5', 'HalfCheetah-v5', 'Hopper-v5', 'Humanoid-v5', 'HumanoidStandup-v5', 'Swimmer-v5', 'Walker2d-v5'])
def test_reset_info(env_id):
    """Verify that the environmet returns info at `reset()`"""
    env = gym.make(env_id)
...
# Note: the max height used to be wrong in the documention.
@pytest.mark.parametrize("version", ['v5'])
...
def test_model_object_count(version):
    """Verify that all the objects of the model are loaded, mostly usefull for using non-mujoco simulator."""
    env = gym.make(f'Ant-{version}')
