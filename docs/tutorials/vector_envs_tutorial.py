...
class A2C(nn.Module):
    ...
    def get_losses(self, rewards: torch.Tensor, action_log_probs: torch.Tensor, value_preds: torch.Tensor,
                   entropy: torch.Tensor, masks: torch.Tensor, gamma: float, lam: float, ent_coef: float, 
...
envs = gym.vector.AsyncVectorEnv([
    lambda: gym.make('LunarLander-v2', gravity=-10.0, enable_wind=True, wind_power=15.0, 
                     turbulence_power=1.5, max_episode_steps=600),
    
    lambda: gym.make('LunarLander-v2', gravity=-9.8, enable_wind=True, wind_power=10.0, 
                     turbulence_power=1.3, max_episode_steps=600),
    
    lambda: gym.make('LunarLander-v2', gravity=-7.0, enable_wind=False, 
...
for sample_phase in tqdm(range(n_updates)):
    ...
    critic_loss, actor_loss = agent.get_losses(ep_rewards, ep_action_log_probs, ep_value_preds, entropy,
