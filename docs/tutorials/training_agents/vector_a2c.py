...
class A2C(nn.Module):
    ...
    def forward(self, x: np.ndarray) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        state_values = self.critic(x).squeeze(-1)  # shape: [n_envs]
        action_logits_vec = self.actor(x)  # shape: [n_envs, n_actions]
        return state_values, action_logits_vec

    def get_state_value_pred(self, x: np.ndarray) -> torch.Tensor:
        """
        ...
        Returns:
            state_values: A tensor with the state values, with shape [n_envs].
        """
        return self.critic(torch.Tensor(x).to(self.device)).squeeze(-1)  # shape: [n_envs]
    ...
    def get_losses(
            ...
            action_log_probs: torch.Tensor,  # Shape [T, B]
            value_preds: torch.Tensor,  # Shape [T+1, B]
            entropies: torch.Tensor,  # Shape [T, B]
            not_done_masks: torch.Tensor,  # Shape [T+1, B]
            gamma: float,
            lam: float,
            ent_coef: float,
            device: torch.device,
        ...
        Args:
            ...
            value_preds: A tensor with the state value predictions for each time step in the episode, with shape [n_steps_per_update + 1, n_envs].
            entropies: Entropy for each time step in the episode, with shape [n_steps_per_update, n_envs].
            not_done_masks: A tensor with the masks for each time step in the episode, with shape [n_steps_per_update, n_envs].
            gamma: The discount factor.
            lam: The GAE hyperparameter. (lam=1 corresponds to Monte-Carlo sampling with high variance and no bias,
                                          and lam=0 corresponds to normal TD-Learning that has a low variance but is biased
                                          because the estimates are generated by a Neural Net).
            ent_coef: Entropy significance parameter.
        ...
        for t in reversed(range(T)):
            # No need for termination masks anymore since setting state values to 0 on terminated episodes is
            # refactored to another part of the code.
            td_error = rewards[t] + gamma * value_preds[t + 1] - value_preds[t]

            # If at step t the episode is either terminated or truncated,
            # the next step t+1 is technically step 1 of a new episode.
            # There is no mathematical validity of computing an advantage for no action performed at all
            # between state V_done of one episode and V_1 of another.
            # Done masks will drop the advantage there to 0.
            gae = ((td_error + gamma * lam * gae) * not_done_masks[t]).detach()
            advantages[t] = gae

        # calculate the loss of the minibatch for actor and critic
        state_values = value_preds[:T]
        # Regress V to a fixed target, do not backprop through the GAE computation loop.
        critic_loss = (state_values - advantages - state_values.detach()).pow(2).mean()

        # give a bonus for higher entropy to encourage exploration
        actor_loss = -(advantages * action_log_probs).mean() - ent_coef * entropies.mean()
...
final_not_done_masks = torch.ones(n_envs, device=device)
...
for sample_phase in tqdm(range(n_updates)):
    ...
    entropies = torch.zeros(n_steps_per_update, n_envs, device=device)
    not_done_masks = torch.zeros(n_steps_per_update + 1, n_envs, device=device)
    continue_masks = torch.ones(n_envs, device=device)

    # at the start of training reset all envs to get an initial state
    if sample_phase == 0:
        states, info = envs_wrapper.reset(seed=42)

    not_done_masks[0] = final_not_done_masks
    ...
    for step in range(n_steps_per_update):
        ...
        # Terminal states have default state value of 0.
        # Multiply with masks to enforce this definition.
        ep_value_preds[step] = state_value_preds * continue_masks
        ep_action_log_probs[step] = action_log_probs
        entropies[step] = entropy * not_done_masks[step]

        # perform the action A_{t} in the environment to get S_{t+1} and R_{t+1}
        # If the episode has previously terminated or truncated the step will reset the environment
        states, rewards, terminated, truncated, infos = envs_wrapper.step(
            actions.cpu().numpy()
        )
        ep_rewards[step] = torch.tensor(rewards, device=device)

        # The state from the step call is s_{t+1} in the current iteration of the loop and turns into s_t
        # in the next iteration.
        # Therefore the current terminated and truncated signals are for the currently s_{t+1} state.
        # The continue_masks will enforce state values during the next iteration to be 0 regardless of critic output,
        # since state values are 0 on terminated episodes by default.
        # Done masks will prevent advantage flow from done episodes to the first steps of the next ones.
        continue_masks = torch.tensor([not term for term in terminated],
                                      device=device)

        # If s_{t+1} is final, prevent carry-over of values into s_1 of the next episode.
        not_done_masks[step + 1] = torch.tensor([not (term or trun) for term, trun in zip(terminated, truncated)],
                                                device=device)

    # Bootstrap value for truncated episodes.
    ep_value_preds[n_steps_per_update] = agent.get_state_value_pred(states) * continue_masks
    final_not_done_masks = not_done_masks[n_steps_per_update]
    ...
    entropies_list.append(entropies.detach().mean().cpu().numpy())
