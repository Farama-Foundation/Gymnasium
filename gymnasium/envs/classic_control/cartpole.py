...
class CartPoleVectorEnv(VectorEnv):
    ...
    def __init__(
        self,
        num_envs: int,
        max_episode_steps: int = 0,  # No timelimit by default
        ...
        # Modified during step
        self.state = None
        self.env_steps = np.zeros(num_envs, dtype=np.int32)
        self.autoreset_env = np.zeros(num_envs, dtype=np.bool_)
    ...
    def step(
        ...
        # Autoreset
        if np.any(self.autoreset_env):
            episode_over_idx = np.where(self.autoreset_env)[0]

            self.state[:, episode_over_idx] = self.np_random.uniform(
                low=self.low, high=self.high, size=(4, np.sum(self.autoreset_env))
            ).astype(np.float32)
            self.env_steps[episode_over_idx] = 0

        # `|` is `np.logical_or`, `state[0]` == x, `state[2]` == theta
        ...
        return self.state.T, rewards, terminations, truncations, {}
    ...
    def reset(
        ...
        *,
        ...
        self.autoreset_env = np.zeros(self.num_envs, dtype=np.bool_)
