...
class FunctionalJaxEnv(gym.Env, Generic[StateType]):
    ...
    def __init__(
        ...
        self.params = func_env.get_default_params(**kwargs)
        ...
        if self.render_mode is not None:
            self.render_state = self.func_env.render_init(params=self.params)
        else:
    ...
    def reset(self, *, seed: int | None = None, options: dict | None = None):
        ...
        obs = self.func_env.observation(self.state, rng, params=self.params)
    ...
    def step(self, action: ActType):
        ...
        next_state = self.func_env.transition(self.state, action, rng, params=self.params)
...
class FunctionalJaxVectorEnv(
    ...
    def __init__(
        ...
        self.func_env = func_env
        self.params = func_env.get_default_params() if params is None else params
    ...
    def step(self, action: ActType):
        ...
        next_state = self.func_env.transition(self.state, action, rng, params=self.params)
        reward = self.func_env.reward(self.state, action, next_state, rng, params=self.params)
        ...
        info = self.func_env.transition_info(self.state, action, next_state, params=self.params)
        ...
        if jnp.any(self.prev_done):
            ...
            def update_reset_leaves(state_leaf, reset_leaf):
                if not (hasattr(state_leaf, "shape") and hasattr(reset_leaf, "shape")):
                    return state_leaf
                if state_leaf.ndim == 0 or reset_leaf.ndim == 0:
                    return state_leaf
                if (
                    state_leaf.shape[0] != self.num_envs
                    or reset_leaf.shape[0] != reset_count
                ):
                    return state_leaf
                return state_leaf.at[to_reset].set(reset_leaf)

            next_state = jax.tree_util.tree_map(
                update_reset_leaves, next_state, new_initials
            )
